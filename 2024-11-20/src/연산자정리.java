/*
 *  데이터저장 (변수) ====> 요청 처리 ========> 화면 출력
 *                    연산자/제어문
 *                   ===========
 *  단항연산자
 *    증감연산자 / 형변환연산자 / 부정연산자
 *                         =>조건문 / 반복문 조건 => 제어문에서 많이 사용함
 *                            |(boolean으로 사용이 가능)
 *                             |true=> false
 *                             |false
 *              => 실수 => 정수
 *               (데이터형)변수 |값
 *              => 크기 순서
 *                    DownCasting        UpCasting
 *                    <=======           =======>
 *                    강제 형변환            자동 형변환
 *               byte < short <int < long <float < double
 *                      char
 *                변수 => 작은 데이터는 저장이 가능
 *                long a=10;
 *                       -- int데이터 => 자동으로 long으로 변경
 *                       --10 => 10L
 *                int a='A'
 *                      ---int데이터
 *                **주의점으로 큰데이터는 첨부가 불가능하다.
 *                int a=10.0 => 오류발생
 *                    =======큰 데이터
 *                ** float/double => f/F
 *                ** int/long => l/L
 *                ** 형변환 => 클래스 형변환
 *    ++ , == : 1씩 증감 for 문일때
 *    
 *  이항연산자
 *   산술연산자 ( + , - , * , / , % )
 *     ** 산술규칙
 *     1)같은 데이터형만 연산이 가능
 *      long + int
 *            =====
 *            long변경 => 자동으로 큰데이터형으로 변경후에 연산
 *           =>long+long
 *           예) 10.5 + 10
 *                     ===
 *                     10.0 => 10.5+10.0 ==> 20.5
 *               'A' + 10
 *               ===
 *               65======> 65+10+75
 *      2)int이하 데이터형을 연산 => 결과값이 int
 *        ======= byte / short / char
 *        byte + byte = int
 *        char + char = int
 *        short + char +byte = int
 *      3)char는 연산시에 자동으로 정수형으로 변경후에 처리
 *      **연산처리
 *        + : 더하기 , 문자열결합
 *                   =======
 *                   5+"5" => "55"
 *                   "5"+5 => "55"
 *        / : 0으로 나눌 수 없다 => 오류발생
 *            정수/정수=정수
 *            ========== 5/2=2
 *        % : 나머지
 *            부호 ( + , - )
 *            5%2 = 1
 *            -5%2 = -1
 *            -5%-2 = -1
 *            -----
 *            5%-2 = 1
 *            => 왼쪽부호 확인
 *        ***가장 많이 사용되는 연산
 *           => 합 / 평균
 *           => 웹프로그램 : 로직이 없다 ,그래서 흐름만 잘 기억하면 된다.
 *   ========================== 조건문/반복문 => 결과값 : true / false
 *   비교연산자
 *    문자열은 비교할 수 없다 / 문자.숫자,논리
 *    => == : 같다
 *       !  : 같지 않다
 *       <  : 작다 ===> 왼쪽 기준
 *       >  : 크다
 *      >=  : 크거나 같다 : > || ==
 *      <=  : 작거나 같다 : < || ==
 *      
 *      'A'=='a' ==> 65==97
 *      => if(부정연산자/비교연산자/논리연산자) => 조건 / 제어문에 들어가는 것들이다.
 *      => 평문이라 내가 원하는 위치로 돌아가지 못한다 / 제어문을 사용해서 바꿀 수 있다.
 *      ===================================
 *      => 조건문 : 필요시 사용 , 불필요시 건너뛴다
 *      => 반복문 : 게임 
 *      =========================================== 흐름 제어
 *   논리연산자
 *         && => 범위 포함 (기간)
 *         char a='B'
 *         a>='A' && a<='Z'
 *         || => 범위밖에 있는 경우
 *         a<'A' || a>'Z'
 *         
 *         int score;
 *         score<0 || score>100 => 다시 입력하시오
 *         (조건) && (조건)
 *         ----     ----
 *          |        |
 *          ----------
 *              |
 *             결과값 => true/false
 *           && 조건 두개가 true일때만 true => 나머지는 false
 *           ***&& 앞에 있는 조건이 false => 뒤에 조건 처리하지 않는다
 *              =>효율적인 연산 
 *            (조건)  ||  (조건)
 *            -----      -----
 *              |          |
 *              ------------
 *                   |
 *                 결과값 = > true/false
 *             || 조건중에 한개이상의 true가 있으면 true
 *             ***|| 앞에 있는 조건이 true => 뒤에 조건 처리하지 않는다
 *   =================================================== 
 *  대입연산자
 *  삼항연산자
 *  => 조건문
 *  => 변수 / 연산자 / 제어문 => 문법 , 형식 , 동작 순사 끝날때까지 계속 한다.(잘 알아두는게 좋음)
 *       =>1. 배열 , 2. 메소드 , 3. 객체지향 , 4. JDBC
 */
//점수 => random => 60점이상 짝수
public class 연산자정리 {

	public static void main(String[] args) {
		// TODO Auto-generated method stub
        //점수
		int score=(int)(Math.random()*101);
		// 0~100까지 사용하겠단 의미
		System.out.println("score:"+score);
		
		boolean bCheck=score>=60 && score%2==0;
		//score%2==0; 짝수 조건
		System.out.println("bCheck="+bCheck);
		
		/*
		 *  score>=90이상 => score<95 범위 안에 && 범위 밖에 || 사용
		 *  
		 */
		char sex='W';
		int jumsu=87;
		
		bCheck=(sex=='W'||jumsu%2==0);
		System.out.println(bCheck);
		
	}

}
