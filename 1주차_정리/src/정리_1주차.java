 ...
 *              데이터형 변수명;
 *            상수)
 *              final 데이터형 변수명=10; => 오류발생(값을 안주면 오류가 발생한다.)
 *              => 배열의 갯수 => 상수
 *              => 상수 : 데이터형의 최대값,최소값
 *        2) 메모리 크기 결정
 *           => 기본형 / 참조형
 ...
 *                       => 표기법 '' => 한글자 설정
 *                                --
 *                  =>한글자만 저장이 가능 => 불편하다 => 여러개 동시에 저장으로 바뀜
 *                                                ---------------------
 *                                                 |String : 문자열 ""
 *                                               
 *              실수형
 ...
 *                       ''=> 붙이면 null (값이 없는 상태)
 *                long l=10L;
 *                => 대입하는 수가 21억 4천 ==> long형으로 변형해야한다.
 *              2. 실수형
 *              double d=10.0D => d 는 생략이 가능
 *              3. 논리형
 ...
 *                           boolean b=false;
 *                           !b=> true
 *                           => 조건문에서 사용 if,for,while 처럼 제어문에서 주로 사용한다.
 *                        형변환 연산자 50page
 *                          =>데이터 크기 확인
 *                          byte < char < int < long < double **boolean은 형변환이 안된다. 여기 나오는 것들만 가능!
 ...
 *                                -5%-2=-1
 *                                
 *                          2.비교연산자 => 모든 결과값이 ***boolean => whrjsans
 *                          == 같다
 *                          !=같지 않다
 *                          -------------------boolean포함
 *                          < 작다
 ...
 *                          =========
 *                            && , || => 병렬연산 => true가 한개 이상이면 처리
 *                            ==직렬연산=> 모든 데이터가 true일때 처리
 *                            (조건) && (조건)
 *                            ----     -----
 *                              |        |
 *                              ----------
 ...
 *                                       int b20;
 *                                       int c=a+b; a와b 값을 더한 값을 c에 대입
 *                                             
 *                                       왼쪽 => 오른쪽으로 연산처리를 하나
 *                                       오른쪽 => 왼쪽 유일하게 대입연산자만 이렇다.
 *                                       그래서 대입연산자는 연산자 마지막 순서이다.
 *                                       *** 산술연산자 : * , / , % , = ,> + , - 
 *                                           비교연산자
 *                                           논리연산자 : && => ||
 *                                           대입연산자
 ...
 *                     모든 프로그램에 변수 , 연산자 , 제어문으로 이루어져 있다.
 *                     
 *       
 *  3장 : 제어문
 *       조건문
 *       = 단일조건문
 *        프로그램은 1)형식 => 흐름 (동작순서) => 문법적용 이 중요하다.
 *                  | 조건문 사용 연산자 => 부정연산자,논리연산자,비교연산자
 *              if(조건문 =>결과값 : ture/false => 건너뛴다)
 *              {
...
public class 정리_1주차 {
	...
	public static void main(String[] args) {
		...
		//모든 데이터형에 대한 정보를 가진 클래스=>Wrapper (앞으로 많이 쓰이게 될 것.)
		int[] arr= {30,40,20,50,10};// 우리가 원하는데로 바꾸는것을 오버라이딩이라고 한다.
