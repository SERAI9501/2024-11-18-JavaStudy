...
public class 문자열_2 {
	...
	public static void main(String[] args) throws Exception {
        ...
        for(int i=1;1<=4;i++)
        {
        	Document doc=(Document) Jsoup.connect("https://www.genie.co.kr/chart/top200?ditc=D&ymd=20241127&hh=10&rtm=Y&pg="+i).get();
        	//곡명 읽기
        	Elements title=doc.select("table.list-wrap a.title");
        	for(int j=0;j<title.size();j++)
        	{
        		if(title.get(j).text().contains(fd)) // %검색어% = 오라클 방식
